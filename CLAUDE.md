...
### Core principles
1. **RELAY-IS-DUMB (Rule 47 / ADR-002)** — The relay may compute commodity/industry-standard metrics (win probability, xG, Elo, factual classification) but NEVER outputs that function as watch recommendations — regardless of label or data type. It may never compute FIELD's proprietary composite scores (drama score, watch value) or any value where receiving it answers "should I watch this game?" The test: could a neutral data vendor (ESPN, FiveThirtyEight) publish this value? Yes → relay-permitted. No → client-only. **This is one of two independent tests, not the whole rule** — passing it does not clear a value for autonomous push. The relay must also serve on pull only, never autonomously push a value-based alert (Rule A, the actual RUWT operative boundary per the 2026-07-07 re-analysis) — a genuine commodity metric like win probability still violates RUWT if pushed unprompted. See ADR-002-CONTEXT.md Rules A and F (Rule F added 2026-07-20) for the full governing principles. "Arithmetic and classification ONLY" is the shorthand; commodity-or-proprietary AND pull-or-push are the actual two-part rule. This is the single most important rule in this repo.
