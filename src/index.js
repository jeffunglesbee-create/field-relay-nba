...
const MLB_STATS_API_ALLOWED_PREFIXES = [
    ...
    '/standings',   // /standings — Pythagorean expected record (mlbPythagInit in field.js)
];
...
// API-Sports.io integration REMOVED 2026-06-30 — subscription lapsed (free tier,
// no current-season access). Former host: api-sports.io. See HANDOFF for removal log.
...
// lapsed; these status-code mappers had zero remaining callers after the
// June 26 2026 migration to ESPN/NBA-CDN/NHLE). adaptBasketball (api-sports
// basketball adapter) removed alongside them for the same reason.
...
// adaptApiNba / adaptHockey / adaptBaseball / adaptAFL / parseFootballStats /
// deriveManAdvantage / adaptFootball — REMOVED 2026-06-30 (api-sports.io
// subscription lapsed; these adapters had zero remaining callers after the
// June 26 2026 migration to ESPN/NBA-CDN/NHLE). See HANDOFF for removal log.
...
async function handleV2Games(url, env, ctx) {
    ...
    // API-Sports.io removed 2026-06-30 — subscription lapsed to free tier
    // (no current-season access). Every V2_LEAGUES entry now resolves via an
    // early-return branch above (espnSource/espnLeague/nbaSource/nhleSource).
    // This is a defensive terminal case only — reached if a future sport is
    // added to V2_LEAGUES without one of those source flags set.
    return new Response(
        JSON.stringify({ error: `No live source configured for sport: ${sport}`, sport, date, games: [], count: 0 }),
        { status: 501, headers: { ...CORS, 'Content-Type': 'application/json' } }
    );
}

// /v2/standings — REMOVED 2026-06-30 (api-sports.io subscription lapsed; route unused by client). See HANDOFF.
...
// fetchWCInjuries — REMOVED 2026-06-30 (api-sports.io subscription lapsed). See HANDOFF.
...
export default {
    ...
    async fetch(request, env, ctx) {
        ...
        if (pathname.startsWith('/wc/')) {
            ...
            // /wc/injuries + /wc/injuries/refresh — REMOVED 2026-06-30 (api-sports.io subscription lapsed). See HANDOFF.
        ...
        // nested INSIDE prefix-guarded blocks whose own startsWith() condition
        // their /admin/... path can never satisfy -- /admin/wc/bsd-backfill was
        // sitting inside `if (pathname.startsWith('/wc/'))` (pre-existing, not
        // touched by any commit this session; confirmed dead code via a live
        // pathname/method echo probe) and this route was accidentally nested the
        // same way inside `if (pathname.startsWith('/archive/'))` when first
        // added, since /archive/backfill-enrich and /archive/game visually
        // neighbor it. Moved both here, top-level, un-nested -- the actual,
        ...
        // POST /fixtures/fetch — REMOVED 2026-06-30 (api-sports.io football fixtures
        // subscription lapsed — free tier rejects season=2026). This was the only
        // data source for the post-WC MLS schedule seed (mls-schedule-seed.yml,
        // July 19-Oct 2026 window) — that workflow needs a replacement source
        // before it runs. See HANDOFF priority list.
        ...
        if (pathname === '/d1/execute' && request.method === 'POST') {
            ...
            const ALLOWED_TABLES = ['odds_history', 'odds_backfill_progress', 'regular_season_games', 'postseason_games', 'change_log', 'analytics_output', 'briefs', 'codex'];
        ...
        // /apisports/* — REMOVED 2026-06-30 (api-sports.io subscription lapsed). See HANDOFF.
